...
import { Volume2, TrendingUp, Wind, Table as TableIcon } from "lucide-react"
...
export default function TechnoPage() {
  ...
  const { data: chartData } = useStationData("techno", chartInterval, granularity)
  ...
  // Schwellenwerte (hier statisch, ggf. aus Config holen)
  const thresholds = { warning: 55, alarm: 60, las: 50, laf: 52 }
  ...
  const alertConfig = {
    normal: {
      ...
      icon: <Volume2 className="w-4 lg:w-5 h-4 lg:h-5 text-pink-400" />,
    ...
    warn: {
      ...
      icon: <Volume2 className="w-4 lg:w-5 h-4 lg:h-5 text-yellow-400" />,
    ...
    alarm: {
      ...
      icon: <Volume2 className="w-4 lg:w-5 h-4 lg:h-5 text-red-400" />,
          ...
          <Card className="bg-white/80 dark:bg-gray-900/60 backdrop-blur-sm border-gray-200 dark:border-gray-700 shadow-xl">
            ...
            <CardContent>
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <ComposedChart data={visibleData} ref={chartRef}>
                    ...
                    <Line
                      ...
                      strokeWidth={3}
                      dot={false}
                      activeDot={{ r: 6, fill: STATION_COLORS.techno.primary }}
                    />
                    <Line
                      type="monotone"
                      dataKey="ws"
              ...
              <div className="flex flex-wrap gap-4 mt-4 text-xs">
                <div className="flex items-center gap-1"><span className="w-3 h-3 rounded-full" style={{background: STATION_COLORS.techno.primary}}></span> Lärmpegel</div>
